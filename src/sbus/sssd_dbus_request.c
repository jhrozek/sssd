                              ...
                              sbus_method_invoker_fn invoker_fn)
{
    ...
    if (invoker_fn) {
        ...
        ret = handler_fn(dbus_req, dbus_req->intf->instance_data);
    }
...
bool
sbus_request_parse_or_finish(struct sbus_request *dbus_req,
                             int first_arg_type,
                             ...)
{
    ...
    if (dbus_message_get_args_valist(dbus_req->message, &error,
                                     first_arg_type, va)) {
        ret = parent_dbus_string_arrays (dbus_req, first_arg_type, va2);
        ...
        /* Log other errors and send them back, this include o.f.d.InvalidArgs */
            ...
            DEBUG(SSSDBG_OP_FAILURE, "Couldn't parse DBus message %s.%s: %s\n",
                  dbus_message_get_interface(dbus_req->message),
                  dbus_message_get_member(dbus_req->message),
                  error.message);
