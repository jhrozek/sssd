                                        ...
                                        struct dp_reply_std *reply)
{
    ...
    name = sss_nss_get_name_from_msg(ctx->domain_info, res->msgs[0]);
    ret = sss_nss_output_name(tmp_ctx, ctx->domain_info, name,
    ...
    if (be->sr_conf.groups != NULL &&
        ...
        for (i = 0; i < res->count && !enabled; i++) {
            ...
            name = sss_nss_get_name_from_msg(ctx->domain_info, msg);
            if (name == NULL) {
                continue;
            }
