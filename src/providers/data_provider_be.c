...
static int data_provider_res_init(struct sbus_request *request);
static int data_provider_go_offline(struct sbus_request *request);
static int data_provider_reset_offline(struct sbus_request *request);
static int data_provider_logrotate(struct sbus_request *request);
...
static int be_get_account_info(struct sbus_request *request);
static int be_pam_handler(struct sbus_request *request);
static int be_sudo_handler(struct sbus_request *request);
static int be_autofs_handler(struct sbus_request *request);
static int be_host_handler(struct sbus_request *request);
static int be_get_subdomains(struct sbus_request *request);
...
static void be_queue_next_request(struct be_req *be_req, enum bet_type type)
{
    ...
    if (dbus_req) {
        ...
        sbus_request_reply_method(dbus_req,
...
static int be_get_subdomains(struct sbus_request *dbus_req)
{
    ...
    be_req = be_req_create(dbus_req, becli, becli->bectx,
...
static int be_get_account_info(struct sbus_request *dbus_req)
{
    ...
    if ((type & BE_REQ_FAST) && becli->bectx->offstat.offline) {
        ...
        ret = sbus_request_reply_method(dbus_req,
                                        DBUS_TYPE_UINT16, &err_maj,
                                        DBUS_TYPE_UINT32, &err_min,
                                        DBUS_TYPE_STRING, &err_msg,
                                        DBUS_TYPE_INVALID);
    ...
    return sbus_request_reply_method(dbus_req,
                                     DBUS_TYPE_UINT16, &err_maj,
                                     DBUS_TYPE_UINT32, &err_min,
                                     DBUS_TYPE_STRING, &err_msg,
                                     DBUS_TYPE_INVALID);
                                    ...
                                    const char *errstr)
{
    ...
    sbus_request_reply(dbus_req, reply);
    dbus_message_unref(reply);
...
{
    struct be_ctx *be_ctx;
    be_ctx = talloc_get_type(sbus_conn_get_private_data(request->conn), struct be_ctx);

    resolv_reread_configuration(be_ctx->be_res->resolv);
    check_if_online(be_ctx);

    return monitor_common_res_init(request);
}
...
{
    errno_t ret;
    struct be_ctx *be_ctx = talloc_get_type(sbus_conn_get_private_data(request->conn),
                                            struct be_ctx);
