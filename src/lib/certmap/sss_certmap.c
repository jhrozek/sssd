                           ...
                           const uint8_t *der_cert, size_t der_size)
{
    ...
    if (ctx->prio_list == NULL) {
        if (ctx->default_matching_rule == NULL) {
            CM_DEBUG(ctx, "No matching rules available.");
            return EINVAL;
        }

        ret = do_match(ctx, ctx->default_matching_rule, cert_content);
        goto done;
    }
                                  ...
                                  char **_filter, char ***_domains)
{
    ...
    if (ctx->prio_list == NULL) {
        ...
        ret = do_match(ctx, ctx->default_matching_rule, cert_content);
        if (ret == 0) {
            ret = get_filter(ctx, ctx->default_mapping_rule, cert_content,
                             &filter);
        }
        goto done;
    }
                     ...
                     struct sss_certmap_ctx **ctx)
{
    ...
    *ctx = talloc_zero(mem_ctx, struct sss_certmap_ctx);
    ...
    ret  = parse_match_rule(*ctx, DEFAULT_MATCH_RULE,
                            &((*ctx)->default_matching_rule));
    if (ret != 0) {
        CM_DEBUG((*ctx), "Failed to parse default matching rule.");
        talloc_free(*ctx);
        *ctx = NULL;
        return ret;
    }
