...
errno_t sssctl_svc_mgmg(enum sss_svc_acct svc_acct)
{
#if defined(HAVE_SYSTEMD_LOGIN) || defined(HAVE_SERVICE)
    switch (svc_acct) {
    case SSS_SVC_START:
        return sssctl_run_command(SSSD_SVC_CMD("start"));
    case SSS_SVC_STOP:
        return sssctl_run_command(SSSD_SVC_CMD("stop"));
    case SSS_SVC_RESTART:
...
bool sssctl_start_sssd(bool force)
{
    ...
    ret = sssctl_svc_mgmg(SSS_SVC_START);
    switch(ret) {
    case EOK:
        return true;
    case ENOSYS:
...
bool sssctl_stop_sssd(bool force)
{
    ...
    ret = sssctl_svc_mgmg(SSS_SVC_STOP);
    switch(ret) {
    case EOK:
        return true;
    case ENOSYS:
...
bool sssctl_restart_sssd(bool force)
{
    ...
    ret = sssctl_svc_mgmg(SSS_SVC_RESTART);
    switch(ret) {
    case EOK:
        return true;
    case ENOSYS:
