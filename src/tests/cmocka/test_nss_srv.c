...
static int test_nss_EPERM_check(uint32_t status, uint8_t *body, size_t blen)
{
    assert_int_equal(status, EPERM);
    assert_int_equal(blen, 0);
...
void test_nss_getpwnam_ex(void **state)
{
    ...
    /* Use flag without permission, expect EPERM */
    mock_input_user_or_group_ex(false, "testuser", SSS_NSS_EX_FLAG_NO_CACHE);
    will_return(__wrap_sss_packet_get_cmd, SSS_NSS_GETPWNAM_EX);

    set_cmd_cb(test_nss_EPERM_check);
    ret = sss_cmd_execute(nss_test_ctx->cctx, SSS_NSS_GETPWNAM_EX,
                          nss_test_ctx->nss_cmds);
    assert_int_equal(ret, EOK);
