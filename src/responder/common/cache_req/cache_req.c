...
{
    if (cr->plugin->validate_domain_fn == NULL) {
        return true;
    }

    return cr->plugin->validate_domain_fn(cr, domain);
}
...
static errno_t cache_req_next_domain(struct tevent_req *req)
{
    ...
    /* We've got some result from previous searches. */
...
static void cache_req_done(struct tevent_req *subreq)
{
    ...
    if (ret == ENOENT && state->result != NULL) {
