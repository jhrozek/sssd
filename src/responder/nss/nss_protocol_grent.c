              ...
              struct sized_string **_name)
{
    ...
    /* Check object class. */
    if (!ldb_msg_check_string_attribute(msg, "objectClass", SYSDB_GROUP_CLASS)) {
    ...
    /* Get fields. */
    name = nss_get_name(domain, msg);
    gid = sss_view_ldb_msg_find_attr_as_uint64(domain, msg, SYSDB_GIDNUM, 0);

    if (name == NULL || gid == 0) {
        DEBUG(SSSDBG_OP_FAILURE, "Incomplete user object for %s[%u]! "
              "Skipping\n", name ? name:"<NULL>", uid);
        return EINVAL;
    }
    ...
    if (ret != EOK) {
        DEBUG(SSSDBG_CRIT_FAILURE, "sized_output_name failed, skipping "
...
#define MEMBERS_OFFSET(name, pw) (2 * sizeof(uint32_t) + name->len + pw->len)
#define MEMBERS_SIZE(body_len, name, pw) (body_len - MEMBERS_OFFSET(name, pw))
                        ...
                        struct cache_req_result *result)
{
    ...
    for (i = 0; i < result->count; i++) {
        ...
        ret = nss_get_grent(tmp_ctx, nss_ctx, result->domain, msg, &gid, &name);
        if (ret != EOK) {
            continue;
        }

        /* TODO for enumeration, check negative cache */
        ...
        SAFEALIGN_SET_UINT32(&body[rp], num_members, &rp);
        ...
        SAFEALIGN_SET_UINT32(&body[sizeof(uint32_t)], num_members, NULL);

        sss_packet_get_body(packet, &body, &body_len);

        num_results++;

        /* TODO for enumeration, do not store mem cache */
        members = body + MEMBERS_OFFSET(name, pwfield);
        members_size = MEMBERS_SIZE(body_len, name, pwfield);
        ret = sss_mmap_cache_gr_store(&nss_ctx->grp_mc_ctx, name, &pwfield,
                                      gid, num_members, members, members_size);
        if (ret != EOK) {
            DEBUG(SSSDBG_MINOR_FAILURE, "Failed to store group %s (%s) "
                  "in mmap cache [%d]: %s!\n", name->str, result->domain->name,
                  ret, sss_strerror(ret));
        }
    }
                         ...
                         struct ldb_result *result)
{
    ...
    for (i = 1; i < result->count; i++) {
        ...
        if (gid == 0) {
            if (posix != NULL && strcmp(posix, "FALSE") == 0) {
                ...
                DEBUG(SSSDBG_CRIT_FAILURE, "Incomplete group object [%s] for "
                      "initgroups! Aborting.\n", ldb_dn_get_linearized(msg->dn));
                ret = EINVAL;
                goto done;
            }
    ...
    if (nss_ctx->initgr_mc_ctx) {
        ...
        to_sized_string(&unique_name, cmd_ctx->normalized_name);

        ret = sss_mmap_cache_initgr_store(&nss_ctx->initgr_mc_ctx, &rawname,
                                          &unique_name, num_results,
                                          body + 2 * sizeof(uint32_t));
        if (ret != EOK) {
            DEBUG(SSSDBG_MINOR_FAILURE, "Failed to store initgroups %s (%s) "
                  "in mmap cache [%d]: %s!\n", rawname->str, domain->name,
                  ret, sss_strerror(ret));
        }
    }
