...
static const char *
nss_get_name(struct sss_domain_info *domain,
             struct ldb_message *msg)
{
    const char *name;

    /* If domain has a view associated we return overridden name if possible. */
    if (DOM_HAS_VIEWS(domain)) {
        name = ldb_msg_find_attr_as_string(msg, OVERRIDE_PREFIX SYSDB_NAME, NULL);
        if (name != NULL) {
            return name;
        }
    }
            ...
            struct ldb_message *msg)
{
    ...
    if (DOM_HAS_VIEWS(domain)) {
        gid = ldb_msg_find_attr_as_uint64(msg, OVERRIDE_PREFIX SYSDB_GIDNUM, 0);
        if (gid != 0) {
            return gid;
        }
    }
                       ...
                       struct sss_domain_info *domain)
{
    ...
    shell = sss_view_ldb_msg_find_attr_as_string(domain, msg, SYSDB_SHELL, NULL);
    if (shell == NULL) {
    ...
    if (nss_ctx->allowed_shells) {
        if (strcmp(nss_ctx->allowed_shells[0], "*") == 0) {
            DEBUG(SSSDBG_FUNC_DATA, "The shell '%s' is allowed but does "
                  "not exist. Using fallback\n", shell);
            return nss_ctx->shell_fallback;
        } else {
            for (i = 0; nss_ctx->allowed_shells[i]; i++) {
                if (strcmp(nss_ctx->allowed_shells[i], shell) == 0) {
                    DEBUG(SSSDBG_FUNC_DATA, "The shell '%s' is allowed but "
                          "does not exist. Using fallback\n", shell);
                    return nss_ctx->shell_fallback;
                }
            }
        }
    ...
    DEBUG(SSSDBG_FUNC_DATA, "The shell '%s' is not "
              ...
              struct sized_string *_shell)
{
    ...
    gecos = sss_view_ldb_msg_find_attr_as_string(domain, msg, SYSDB_GECOS, NULL);
                        ...
                        struct cache_req_result *result)
{
    ...
    for (i = 0; i < result->count; i++) {
        ...
        if (ret != EOK) {
            DEBUG(SSSDBG_MINOR_FAILURE, "Failed to store user %s (%s) "
