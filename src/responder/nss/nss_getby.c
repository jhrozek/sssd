                              ...
                              enum sss_mc_type type)
{
    ...
    DEBUG(SSSDBG_CRIT_FAILURE, "Internal failure in memory cache code: "
                      ...
                      enum sss_mc_type type)
{
    ...
    for (dom = rctx->domains;
        ...
        if (name != NULL) {
            ...
            if (ret != EOK) {
                DEBUG(SSSDBG_OP_FAILURE, "Unable to create sized name "
                      "[%d]: %s\n", ret, sss_strerror(ret));
                return ret;
            }
            ...
            if (ret != EOK) {
                DEBUG(SSSDBG_OP_FAILURE, "Unable to delete '%s' from domain "
                      "'%s' memory cache!\n", name, dom->name);
                continue;
            }
        } else if (id != 0) {
            ...
            if (ret != EOK) {
                DEBUG(SSSDBG_OP_FAILURE, "Unable to delete '%u' from domain "
                      "'%s' memory cache!\n", id, dom->name);
                continue;
            }
        } else {
...
struct tevent_req *
nss_get_by_name_send(TALLOC_CTX *mem_ctx,
                     struct tevent_context *ev,
                     struct cli_ctx *cli_ctx,
                     enum cache_req_type type,
                     enum sss_mc_type memcache)
{
    struct nss_get_by_name_state *state;
    struct cache_req_data *data;
    struct tevent_req *subreq;
    struct tevent_req *req;
    errno_t ret;

    req = tevent_req_create(mem_ctx, &state, struct nss_get_by_name_state);
    if (req == NULL) {
    ...
    state->rawname = nss_protocol_parse_name(cli_ctx);
    ...
    tevent_req_set_callback(subreq, nss_get_by_name_done, req);
...
static void nss_get_by_name_done(struct tevent_req *subreq)
{
    ...
    state = tevent_req_data(req, struct nss_get_by_name_state);
    ...
    /* We don't care about error here since we can't do anything about it. */
    memcache_delete_entry(state->nss_ctx, state->rctx, state->result->domain,
                          state->result->lookup_name, 0, state->memcache);
...
errno_t
nss_get_by_name_recv(TALLOC_CTX *mem_ctx,
                     struct tevent_req *req,
                     ...
                     const char **_rawname)
{
    ...
    if (_rawname != NULL) {
        *_rawname = talloc_steal(mem_ctx, state->rawname);
    }

    return EOK;
}

struct nss_get_by_id_state {
    struct cli_ctx *cli_ctx;
    struct nss_ctx *nss_ctx;
    struct resp_ctx *rctx;
    enum sss_mc_type memcache;
...
struct tevent_req *
nss_get_by_id_send(TALLOC_CTX *mem_ctx,
                   struct tevent_context *ev,
                   struct cli_ctx *cli_ctx,
                   enum cache_req_type type,
                   enum sss_mc_type memcache)
{
    struct nss_get_by_id_state *state;
    struct cache_req_data *data;
    struct tevent_req *subreq;
    struct tevent_req *req;
    errno_t ret;

    req = tevent_req_create(mem_ctx, &state, struct nss_get_by_id_state);
    if (req == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to create tevent request!\n");
        return NULL;
    }
    ...
    tevent_req_set_callback(subreq, nss_get_by_id_done, req);
...
static void nss_get_by_id_done(struct tevent_req *subreq)
{
    ...
    req = tevent_req_callback_data(subreq, struct tevent_req);
    state = tevent_req_data(req, struct nss_get_by_id_state);

    ret = cache_req_recv(state, subreq, &state->result);
    talloc_zfree(subreq);
...
errno_t nss_get_by_id_recv(TALLOC_CTX *mem_ctx,
                           struct tevent_req *req,
                           struct cache_req_result **_result)
{
    struct nss_get_by_id_state *state;
    ...
    if (_result != NULL) {
        *_result = talloc_steal(mem_ctx, state->result->ldb_result);
    }
