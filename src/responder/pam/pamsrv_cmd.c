 ...
 * authentication was successful */
    ...
    /* Add informational messages to PAM reply for cases where the user
     * is expired or locked out
     */
    pamsrv_exp_warn(pd,
    ...
    ret = pamsrv_reply_packet(prctx->creq, pd,
                              sss_packet_get_cmd(prctx->creq->in),
                              &prctx->creq->out);
    if (ret != EOK) {
        goto done;
    }
