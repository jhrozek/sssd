...
static int get_monitor_version(struct sbus_request *request)
{
    dbus_uint16_t version = MONITOR_VERSION;

    return sbus_request_reply_method(request,
...
static int client_registration(struct sbus_request *request)
{
    ...
    data = sbus_conn_get_private_data(request->conn);
    ...
    sbus_request_reply_method (request,
                               DBUS_TYPE_UINT16, &version,
                               DBUS_TYPE_INVALID);
